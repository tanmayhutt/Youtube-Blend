...
# Ensure TTL indexes exist for state and auth codes
try:
    ...
    logger.exception("Failed to ensure TTL indexes on auth collections")
...
async def join_comparison(comparison_id: str):
    ...
    if comparison.get('expires_at') and comparison['expires_at'] < datetime.utcnow():
        raise HTTPException(status_code=400, detail="Comparison link has expired")
...
async def auth_complete_fallback(code: str = None, next: str = None):
        ...
        # Minimal HTML that sets token in localStorage and redirects
        ...
        <html>
            ...
            <body>
                <script>
                    ...
                    window.location = '{frontend}{redirect_target}';
                </script>
